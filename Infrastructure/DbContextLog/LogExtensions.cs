namespace Microsoft.Samples.EFLogging
{
    ...
    //扩展DBSet,DBContext，输出EFCore的sql执行语句
    public static class DbContextLoggingExtensions
    {
        public static void ConfigureLogging<TEntity>(this DbSet<TEntity> dbSet, Action<String> logger, Func<string, LogLevel, bool> filter) where TEntity : class
        {
            var serviceProvider = dbSet.GetInfrastructure<IServiceProvider>();
            var loggerFactory = (ILoggerFactory)serviceProvider.GetService(typeof(ILoggerFactory));

            LogProvider.CreateOrModifyLoggerForDbContext(dbSet.GetType().BaseType, loggerFactory, logger, filter);
        }
